...
function SWEP:PrimaryAttack()
    ...
    if self.MultiMode and self:GetOwner():KeyDown(IN_USE) then
        if self:GetFireMode() == "semi" then
            ...
            self:GetOwner():PrintMessage(HUD_PRINTCENTER, FrenchRP.getPhrase("switched_burst"))
        elseif self:GetFireMode() == "burst" then
            self:SetFireMode("auto")
            self.Primary.Automatic = true
            self:GetOwner():PrintMessage(HUD_PRINTCENTER, FrenchRP.getPhrase("switched_fully_auto"))
        elseif self:GetFireMode() == "auto" then
            self:SetFireMode("semi")
            self.Primary.Automatic = false
            self:GetOwner():PrintMessage(HUD_PRINTCENTER, FrenchRP.getPhrase("switched_semi_auto"))
        end
    ...
    self:GetOwner():ViewPunch(Angle(util.SharedRandom("FrenchRP_CSBase" .. self:EntIndex() .. "Mag" .. self:GetTotalUsedMagCount() .. "p" .. self:Clip1(), -1.2, -1.1) * self.Primary.Recoil, util.SharedRandom("FrenchRP_CSBase" .. self:EntIndex() .. "Mag" .. self:GetTotalUsedMagCount() .. "y" .. self:Clip1(), -1.1, 1.1) * self.Primary.Recoil, 0))
end
...
if CLIENT then
    function SWEP:ViewModelDrawn(vm)
        if self.FrenchRPViewModelBoneManipulations and not self:GetReloading() then
            self:UpdateFrenchRPBones(vm, self.FrenchRPViewModelBoneManipulations)
        else
            self:ResetFrenchRPBones(vm)
        end
    end

    function SWEP:UpdateFrenchRPBones(vm, manipulations)
    ...
    function SWEP:ResetFrenchRPBones(vm)
        if not IsValid(vm) or not vm:GetBoneCount() then return end
        for i = 0, vm:GetBoneCount() - 1 do
...
hook.Add("SetupMove", "FrenchRP_WeaponSpeed", function(ply, mv)
    local wep = ply:GetActiveWeapon()
    if not IsValid(wep) or not wep.FrenchRPBased or not wep.GetIronsights or not wep:GetIronsights() then return end
