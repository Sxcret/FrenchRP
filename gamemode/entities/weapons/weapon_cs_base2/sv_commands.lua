...
function meta:dropDRPWeapon(weapon)
    ...
    hook.Call("onFrenchRPWeaponDropped", nil, self, ent, weapon)
...
local function DropWeapon(ply)
    ...
    if not IsValid(ent) or not ent:GetModel() or ent:GetModel() == "" then
        FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("cannot_drop_weapon"))
        return ""
    end
...
FrenchRP.defineChatCommand("drop", DropWeapon)
FrenchRP.defineChatCommand("dropweapon", DropWeapon)
FrenchRP.defineChatCommand("weapondrop", DropWeapon)
...
FrenchRP.hookStub{
    name = "onFrenchRPWeaponDropped",
    description = "When a player drops a weapon. Use this hook (in combination with PlayerPickupFrenchRPWeapon) to store extra information about a weapon. This hook cannot prevent weapon dropping. If you want to prevent weapon dropping, use canDropWeapon instead.",
    parameters = {
        {
            name = "ply",
...
FrenchRP.hookStub{
    name = "PlayerPickupFrenchRPWeapon",
