...
function SWEP:PrimaryAttack()
    ...
    if not canPickup then
        if message then FrenchRP.notify(self:GetOwner(), 1, 4, message) end
        return
    end
...
function SWEP:SecondaryAttack()
    ...
    if item <= 0 then
        FrenchRP.notify(self:GetOwner(), 1, 4, FrenchRP.getPhrase("pocket_no_items"))
        return
    end
