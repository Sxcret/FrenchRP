...
function SWEP:PrimaryAttack()
    ...
    if not IsValid(ent) or ent.FrenchRPCanLockpick == false then return end
    ...
    local endDelta = hook.Call("lockpickTime", nil, self:GetOwner(), ent) or util.SharedRandom("FrenchRP_Lockpick" .. self:EntIndex() .. "_" .. self:GetTotalLockpicks(), 10, 30)
...
function SWEP:Think()
    ...
    if CurTime() >= self:GetNextSoundTime() then
        ...
        local snd = {1,3,4}
        self:EmitSound("weapons/357/357_reload" .. tostring(snd[math.Round(util.SharedRandom("FrenchRP_LockpickSnd" .. CurTime(), 1, #snd))]) .. ".wav", 50, 100)
    end
...
function SWEP:DrawHUD()
    ...
    draw.DrawNonParsedSimpleText(FrenchRP.getPhrase("picking_lock") .. self.Dots, "Trebuchet24", w / 2, y + height / 2, Color(255, 255, 255, 255), 1, 1)
end
