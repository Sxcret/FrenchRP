...
if SERVER then
    ...
    util.AddNetworkString("FrenchRP_keypadData")
end

SWEP.Base = "weapon_base"

SWEP.PrintName = "Admin Keypad Checker"
SWEP.Author = "FrenchRP Developers"
...
SWEP.Spawnable = true
SWEP.AdminOnly = true
SWEP.Category = "FrenchRP (Utility)"
...
local function getTargets(keypad, keyPass, keyDenied, delayPass, delayDenied)
    ...
    for k,v in pairs(numpad.OnDownItems or {}) do
        if v.key == keyPass and v.ply == Owner then
            table.insert(targets, {type = FrenchRP.getPhrase("keypad_checker_entering_right_pass"), name = v.name, ent = v.ent, original = keypad})
        end
        if v.key == keyDenied and v.ply == Owner then
            table.insert(targets, {type = FrenchRP.getPhrase("keypad_checker_entering_wrong_pass"), name = v.name, ent = v.ent, original = keypad})
        end
    end

    for k,v in pairs(numpad.OnUpItems or {}) do
        if v.key == keyPass and v.ply == Owner then
            table.insert(targets, {type = FrenchRP.getPhrase("keypad_checker_after_right_pass"), name = v.name, delay = math.Round(delayPass, 2), ent = v.ent, original = keypad})
        end
        if v.key == keyDenied and v.ply == Owner then
            table.insert(targets, {type = FrenchRP.getPhrase("keypad_checker_after_wrong_pass"), name = v.name, delay = math.Round(delayDenied, 2), ent = v.ent, original = keypad})
        end
    end
...
local function getEntityKeypad(ent)
    ...
    for k,v in pairs(ents.FindByClass("sent_keypad")) do
        ...
        if vOwner == entOwner and table.HasValue(doorKeys, v:GetNWInt("keypad_keygroup1")) then
            table.insert(targets, {type = FrenchRP.getPhrase("keypad_checker_right_pass_entered"), ent = v, original = ent})
        end
        if vOwner == entOwner and  table.HasValue(doorKeys, v:GetNWInt("keypad_keygroup2")) then
            table.insert(targets, {type = FrenchRP.getPhrase("keypad_checker_wrong_pass_entered"), ent = v, original = ent})
        end
    end
...
function SWEP:PrimaryAttack()
    ...
    if class == "sent_keypad" then
        ...
        FrenchRP.notify(self:GetOwner(), 1, 4, FrenchRP.getPhrase("keypad_checker_controls_x_entities", #data / 2))
    elseif class == "keypad" then
        data = get_keypad_Info(ent)
        FrenchRP.notify(self:GetOwner(), 1, 4, FrenchRP.getPhrase("keypad_checker_controls_x_entities", #data / 2))
    else
        data = getEntityKeypad(ent)
        FrenchRP.notify(self:GetOwner(), 1, 4, FrenchRP.getPhrase("keypad_checker_controlled_by_x_keypads", #data))
    end

    net.Start("FrenchRP_keypadData")
        net.WriteTable(data)
