...
SWEP.IsFrenchRPUnarrestStick = true
...
function SWEP:PrimaryAttack()
    ...
    if not IsValid(ent) or not ent:IsPlayer() or (self:GetOwner():EyePos():Distance(ent:GetPos()) > 90) or not ent:getFrenchRPVar("Arrested") then
        return
    end

    local canUnarrest, message = hook.Call("canUnarrest", hookCanUnarrest, self:GetOwner(), ent)
    if not canUnarrest then
        if message then FrenchRP.notify(self:GetOwner(), 1, 5, message) end
        return
    end

    ent:unArrest(self:GetOwner())
    FrenchRP.notify(ent, 0, 4, FrenchRP.getPhrase("youre_unarrested_by", self:GetOwner():Nick()))

    if self:GetOwner().SteamName then
        FrenchRP.log(self:GetOwner():Nick() .. " (" .. self:GetOwner():SteamID() .. ") unarrested " .. ent:Nick(), Color(0, 255, 255))
    end
end

function SWEP:startFrenchRPCommand(usrcmd)
