...
local function ramDoor(ply, trace, ent)
    ...
    if not allowed then
        FrenchRP.notify(ply, 1, 5, FrenchRP.getPhrase("warrant_required"))
...
local function ramProp(ply, trace, ent)
    ...
    if not canRam(Owner) then
        FrenchRP.notify(ply, 1, 5, FrenchRP.getPhrase(GAMEMODE.Config.copscanunweld and "warrant_required_unweld" or "warrant_required_unfreeze"))
        return false
    end
...
function SWEP:PrimaryAttack()
    ...
    self:GetOwner():ViewPunch(Angle(-10, math.Round(util.SharedRandom("FrenchRP_DoorRam" .. self:EntIndex() .. "_" .. self:GetTotalUsedMagCount(), -5, 5)), 0))
end
...
hook.Add("SetupMove", "FrenchRP_DoorRamJump", function(ply, mv)
