...
SWEP.Contact = ""
SWEP.Purpose = ""
SWEP.IsFrenchRPWeaponChecker = true
...
function SWEP:GetStrippableWeapons(ent, callback)
    CAMI.PlayerHasAccess(ent, "FrenchRP_GetAdminWeapons", function(access)
...
function SWEP:PrimaryAttack()
    ...
    if result == "" then
        self:GetOwner():ChatPrint(FrenchRP.getPhrase("no_illegal_weapons", trace.Entity:Nick()))
        return
    end

    self:GetOwner():ChatPrint(FrenchRP.getPhrase("persons_weapons", trace.Entity:Nick()))
...
function SWEP:SecondaryAttack()
    ...
    self:SetEndCheckTime(CurTime() + util.SharedRandom("FrenchRP_WeaponChecker" .. self:EntIndex() .. "_" .. self:GetTotalWeaponChecks(), 5, 10))
...
function SWEP:Reload()
    ...
    if not trace.Entity.ConfiscatedWeapons then
        FrenchRP.notify(self:GetOwner(), 1, 4, FrenchRP.getPhrase("no_weapons_confiscated", trace.Entity:Nick()))
        return
    else
        ...
        FrenchRP.notify(self:GetOwner(), 2, 4, FrenchRP.getPhrase("returned_persons_weapons", trace.Entity:Nick()))
...
function SWEP:Succeed()
    ...
    if result == "" then
        ...
        self:GetOwner():ChatPrint(FrenchRP.getPhrase("confiscated_these_weapons"))
        if string.len(result) >= 126 then
            local amount = math.ceil(string.len(result) / 126)
            for i = 1, amount, 1 do
                self:GetOwner():ChatPrint(string.sub(result, (i-1) * 126, i * 126 - 1))
...
function SWEP:DrawHUD()
    if self:GetIsWeaponChecking() and self:GetEndCheckTime() ~= 0 then
        ...
        draw.DrawNonParsedSimpleText(FrenchRP.getPhrase("checking_weapons") .. self.Dots, "Trebuchet24", w / 2, y + height / 2, Color(255, 255, 255, 255), 1, 1)
    end
end
