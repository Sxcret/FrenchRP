...
function ENT:Draw()
    ...
    local owner = (IsValid(self:Getowning_ent()) and self:Getowning_ent().Name and self:Getowning_ent():Name()) or FrenchRP.getPhrase("unknown")
    local recipient = (self:Getrecipient().Name and self:Getrecipient():Name()) or FrenchRP.getPhrase("unknown")

    surface.SetFont("ChatFont")
    local text = FrenchRP.getPhrase("cheque_pay", recipient) .. "\n" .. FrenchRP.formatMoney(amount) .. "\n" .. FrenchRP.getPhrase("signed", owner)
