...
function ENT:Use(activator, caller)
    ...
    if (IsValid(activator) and IsValid(recipient)) and activator == recipient then
        owner = (IsValid(owner) and owner:Nick()) or FrenchRP.getPhrase("disconnected_player")
        FrenchRP.notify(activator, 0, 4, FrenchRP.getPhrase("found_cheque", FrenchRP.formatMoney(amount), "", owner))
        activator:addMoney(amount)
        self:Remove()
    elseif (IsValid(owner) and IsValid(recipient)) and owner ~= activator then
        FrenchRP.notify(activator, 0, 4, FrenchRP.getPhrase("cheque_details", recipient:Name()))
    elseif IsValid(owner) and owner == activator then
        FrenchRP.notify(activator, 0, 4, FrenchRP.getPhrase("cheque_torn"))
...
function ENT:Touch(ent)
    ...
    if ent:GetClass() ~= "frenchrp_cheque" or self.USED or ent.USED or self.hasMerged or ent.hasMerged then return end
