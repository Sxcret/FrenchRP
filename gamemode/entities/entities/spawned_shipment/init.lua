...
function ENT:Initialize()
    ...
    if self:Getcount() < 1 then
        ...
        if not contents then
            FrenchRP.error("Shipment created with zero or fewer elements.", 2)
        else
            FrenchRP.error(string.format("Some smartass thought they were clever by setting the 'amount' of shipment '%s' to 0.\nWhat the fuck do you expect the use of an empty shipment to be?", contents.name), 2)
        end
    end
end
...
function ENT:StartSpawning()
    ...
    timer.Simple(0, function()
        net.Start("FrenchRP_shipmentSpawn")
