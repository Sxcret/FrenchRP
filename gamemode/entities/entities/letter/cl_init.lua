...
local function ShowLetter(msg)
    ...
    SignButton:SetText(FrenchRP.getPhrase("sign_this_letter"))
    frame:SetPos(ScrW() - 256, ScrH() - 256)
    SignButton:SetSize(256, 256)
    frame:SetSize(256, 256)
    SignButton:SetSkin(GAMEMODE.Config.FrenchRPSkin)
    frame:SizeToContents()
    frame:MakePopup()
    frame:SetKeyboardInputEnabled(false)

    function SignButton:DoClick()
        RunConsoleCommand("_FrenchRP_SignLetter", Letter:EntIndex())
    ...
    hook.Add("HUDPaint", "ShowLetter", function()
        ...
        draw.DrawNonParsedText(LetterMsg .. "\n\n\n" .. FrenchRP.getPhrase("signed", (IsValid(Letter:Getsigned()) and Letter:Getsigned():Nick() or FrenchRP.getPhrase("no_one"))), font, ScrW() * .25 + 20, LetterY + 80, Color(0, 0, 0, LetterAlpha), 0)
