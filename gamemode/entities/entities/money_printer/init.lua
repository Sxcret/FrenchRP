...
function ENT:Destruct()
    ...
    if IsValid(self:Getowning_ent()) then FrenchRP.notify(self:Getowning_ent(), 1, 4, FrenchRP.getPhrase("money_printer_exploded")) end
end

function ENT:BurstIntoFlames()
    if hook.Run("moneyPrinterCatchFire", self) == true then return end

    if IsValid(self:Getowning_ent()) then FrenchRP.notify(self:Getowning_ent(), 0, 4, FrenchRP.getPhrase("money_printer_overheating")) end
    self.burningup = true
...
function ENT:CreateMoneybag()
    ...
    local moneybag = FrenchRP.createMoneyBag(Vector(MoneyPos.x + 15, MoneyPos.y, MoneyPos.z + 15), amount)
