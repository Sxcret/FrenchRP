...
function ENT:Use(activator,caller)
    ...
    if activator ~= Owner then
        ...
        FrenchRP.payPlayer(activator, Owner, self:Getprice())
        FrenchRP.notify(activator, 0, 4, FrenchRP.getPhrase("you_bought", string.lower(FrenchRP.getPhrase("drugs")), FrenchRP.formatMoney(self:Getprice()), ""))
        FrenchRP.notify(Owner, 0, 4, FrenchRP.getPhrase("you_received_x", FrenchRP.formatMoney(self:Getprice()), string.lower(FrenchRP.getPhrase("drugs"))))
    end
    DrugPlayer(caller)
    self.CanUse = false
    self:Remove()
