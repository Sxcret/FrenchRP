module("FrenchRP", package.seeall)
...
local stubLayout = {
    ...
    metatable = istable -- FrenchRP, Player, Entity, Vector, ...
}
...
function stub(tbl)
    ...
    if not isStub then
        error("Invalid FrenchRP method stub! Field \"" .. field .. "\" is invalid!", 2)
    end
...
function hookStub(tbl)
    ...
    if not isStub then
        error("Invalid FrenchRP hook! Field \"" .. field .. "\" is invalid!", 2)
    end
