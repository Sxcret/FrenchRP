...
function FrenchRP.addLanguage(name, tbl)
    local old = rp_languages[name] or {}
    rp_languages[name] = tbl

    -- Merge the language with the translations added by FrenchRP.addPhrase
...
function FrenchRP.addPhrase(lang, name, phrase)
    rp_languages[lang] = rp_languages[lang]  or {}
    rp_languages[lang][name] = phrase
end

function FrenchRP.getPhrase(name, ...)
...
function FrenchRP.getMissingPhrases(lang)
...
local function getMissingPhrases(ply, cmd, args)
    if not args[1] then print("Please run the command with a language code e.g. frenchrp_getphrases \"en\"") return end
    ...
    print(FrenchRP.getMissingPhrases(args[1]))
end
if CLIENT then concommand.Add("frenchrp_getphrases", getMissingPhrases) end

/*---------------------------------------------------------------------------
Chat command translating
---------------------------------------------------------------------------*/
local chatCmdDescriptions = {}
function FrenchRP.addChatCommandsLanguage(lang, tbl)
    chatCmdDescriptions[lang] = chatCmdDescriptions[lang] or {}

    table.Merge(chatCmdDescriptions[lang], tbl)
end

function FrenchRP.getChatCommandDescription(name)
    local cmd = FrenchRP.getChatCommand(name)
...
local function getMissingCmdTranslations()
    local cmds = FrenchRP.getSortedChatCommands()
...
if CLIENT then concommand.Add("frenchrp_translateChatCommands", printMissingChatTranslations) end
