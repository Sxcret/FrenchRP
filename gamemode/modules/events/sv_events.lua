...
local function StormStart()
    for k, v in pairs(player.GetAll()) do
        if v:Alive() then
            v:PrintMessage(HUD_PRINTCENTER, FrenchRP.getPhrase("meteor_approaching"))
            v:PrintMessage(HUD_PRINTTALK, FrenchRP.getPhrase("meteor_approaching"))
        end
    end
end
...
local function StormEnd()
    for k, v in pairs(player.GetAll()) do
        if v:Alive() then
            v:PrintMessage(HUD_PRINTCENTER, FrenchRP.getPhrase("meteor_passing"))
            v:PrintMessage(HUD_PRINTTALK, FrenchRP.getPhrase("meteor_passing"))
        end
    end
end
...
local function StartStorm(ply)
    ...
    FrenchRP.notify(ply, 0, 4, FrenchRP.getPhrase("meteor_enabled"))
end
FrenchRP.definePrivilegedChatCommand("enablestorm", "FrenchRP_AdminCommands", StartStorm)
...
local function StopStorm(ply)
    ...
    timer.Stop("start")
    StormEnd()
    FrenchRP.notify(ply, 0, 4, FrenchRP.getPhrase("meteor_disabled"))
end
FrenchRP.definePrivilegedChatCommand("disablestorm", "FrenchRP_AdminCommands", StopStorm)
...
local function TremorReport()
    ...
    if mag then
        if mag < 6.5 then
            FrenchRP.notifyAll(0, 3, FrenchRP.getPhrase("earthtremor_report", tostring(mag)))
            return
        end
        FrenchRP.notifyAll(0, 3, FrenchRP.getPhrase("earthquake_report", tostring(mag)))
    end
end
