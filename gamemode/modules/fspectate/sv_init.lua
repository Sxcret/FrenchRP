...
local function playerVoice(listener, talker)
    ...
    if not IsValid(listener.FSpectatingEnt) then
        if not FrenchRP or not GAMEMODE.Config.voiceradius or not listener.FSpectatePos then return end
...
local function playerSay(talker, message)
    ...
    if not FrenchRP then return end
    ...
    for _, ply in pairs(player.GetAll()) do
        ...
        if ply.FSpectatePos and talker:GetShootPos():Distance(ply.FSpectatePos) <= 600 and
            ...
            FrenchRP.talkToPerson(ply, team.GetColor(talker:Team()), talker:Nick(), Color(255, 255, 255, 255), message, talker)
            return
        end
