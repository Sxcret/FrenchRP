...
function FrenchRP.createQuestion(question, quesid, ent, delay, callback, fromPly, toPly, ...)
    local newques = { }
...
function FrenchRP.destroyQuestion(id)
...
function FrenchRP.destroyQuestionsWithEnt(ent)
    for k, v in pairs(Questions) do
        if v.Ent == ent then
            FrenchRP.destroyQuestion(v.ID)
        end
    end
end
