...
local function MsgDoVote(msg)
    ...
    panel:SetTitle(FrenchRP.getPhrase("vote"))
    ...
    label:SetPos(5, 25)
    label:SetText(FrenchRP.deLocalise(question))
    ...
    ybutton:SetCommand("!")
    ybutton:SetText(FrenchRP.getPhrase("yes"))
    ...
    nbutton:SetCommand("!")
    nbutton:SetText(FrenchRP.getPhrase("no"))
    ...
    panel:SetSkin(GAMEMODE.Config.FrenchRPSkin)
end
...
local function MsgDoQuestion(msg)
    ...
    function panel:Think()
        self:SetTitle(FrenchRP.getPhrase("time", math.Clamp(math.ceil(timeleft - (CurTime() - OldTime)), 0, 9999)))
        if timeleft - (CurTime() - OldTime) <= 0 then
            panel:Close()
        end
    end
