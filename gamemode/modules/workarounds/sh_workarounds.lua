...
function entity:EmitSound(sound, ...)
    if not sound then FrenchRP.error(string.format("The first argument of the ent:EmitSound call is '%s'. It's supposed to be a string.", tostring(sound)), 3) end
    if string.find(sound, "??", 0, true) then return end
    return EmitSound(self, sound, ...)
end


function FrenchRP.getAvailableVehicles()
...
timer.Simple(3, function()
    ...
    if Skid then
        ...
        SkidCheck would ban me (FPtje, developer of FrenchRP) from your server because I have a
...
if CLIENT then
    ...
    ---------------------------------------------------------------------------*/
    hook.Add("InitPostEntity", "FrenchRP_Workarounds", function()
...
local function msgDeprecated(cmd, ply)
    ...
    FrenchRP.notify(ply, 1, 4, ("This command has been deprecated. Please use 'FrenchRP %s' or '/%s' instead."):format(cmd.alternative, cmd.alternative))
end
