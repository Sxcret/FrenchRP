...
hook.Add("loadCustomFrenchRPItems", "loadAnimations", function()
    Anims[ACT_GMOD_GESTURE_BOW] = FrenchRP.getPhrase("bow")
    Anims[ACT_GMOD_TAUNT_MUSCLE] = FrenchRP.getPhrase("dance")
    Anims[ACT_GMOD_GESTURE_BECON] = FrenchRP.getPhrase("follow_me")
    Anims[ACT_GMOD_TAUNT_LAUGH] = FrenchRP.getPhrase("laugh")
    Anims[ACT_GMOD_TAUNT_PERSISTENCE] = FrenchRP.getPhrase("lion_pose")
    Anims[ACT_GMOD_GESTURE_DISAGREE] = FrenchRP.getPhrase("nonverbal_no")
    Anims[ACT_GMOD_GESTURE_AGREE] = FrenchRP.getPhrase("thumbs_up")
    Anims[ACT_GMOD_GESTURE_WAVE] = FrenchRP.getPhrase("wave")
end)

function FrenchRP.addPlayerGesture(anim, text)
    if not anim then FrenchRP.error("Argument #1 of FrenchRP.addPlayerGesture (animation/gesture) does not exist.", 2) end
    if not text then FrenchRP.error("Argument #2 of FrenchRP.addPlayerGesture (text) does not exist.", 2) end

    Anims[anim] = text
end

function FrenchRP.removePlayerGesture(anim)
    if not anim then FrenchRP.error("Argument #1 of FrenchRP.removePlayerGesture (animation/gesture) does not exist.", 2) end

    Anims[anim] = nil
end

hook.Add("CalcMainActivity", "frenchrp_animations", function(ply, velocity) -- Using hook.Add and not GM:CalcMainActivity to prevent animation problems
...
if SERVER then
    local function CustomAnim(ply, cmd, args)
        ...
        umsg.Start("_FrenchRP_CustomAnim", RP)
        umsg.Entity(ply)
        umsg.Short(Gesture)
        umsg.End()
    end
    concommand.Add("_FrenchRP_DoAnimation", CustomAnim)
    return
end
...
usermessage.Hook("_FrenchRP_CustomAnim", CustomAnimation)
...
local function AnimationMenu()
    ...
    AnimFrame:SetTitle(FrenchRP.getPhrase("custom_animation"))
    ...
    for k,v in SortedPairs(Anims) do
        ...
        button.DoClick = function()
            RunConsoleCommand("_FrenchRP_DoAnimation", k)
        end
    end
    AnimFrame:SetSkin(GAMEMODE.Config.FrenchRPSkin)
end
concommand.Add("_FrenchRP_AnimationMenu", AnimationMenu)
