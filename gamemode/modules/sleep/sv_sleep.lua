...
local function onRagdollArrested(arrestee, _, arrester)
    FrenchRP.toggleSleep(arrestee, "force")

    -- onArrestStickUsed
    local canArrest, message = hook.Call("canArrest", FrenchRP.hooks, arrester, arrestee)
    if not canArrest then
        if message then FrenchRP.notify(arrester, 1, 5, message) end
        return
    end

    arrestee:arrest(nil, arrester)

    FrenchRP.notify(arrestee, 0, 20, FrenchRP.getPhrase("youre_arrested_by", arrester:Nick()))

    if arrester.SteamName then
        FrenchRP.log(arrester:Nick() .. " (" .. arrester:SteamID() .. ") arrested " .. arrestee:Nick(), Color(0, 255, 255))
    end
end

function FrenchRP.toggleSleep(player, command)
    ...
    if not player:Alive() then
        FrenchRP.notify(player, 1, 4, FrenchRP.getPhrase("must_be_alive_to_do_x", "/sleep"))
        return ""
    end

    if (not player.KnockoutTimer or player.KnockoutTimer + KnockoutTime >= CurTime()) and command ~= "force" then
        FrenchRP.notify(player, 1, 4, FrenchRP.getPhrase("have_to_wait", math.ceil((player.KnockoutTimer + KnockoutTime) - CurTime()), "/sleep"))
        return ""
    end

    if player:IsFrozen() then
        FrenchRP.notify(player, 1, 4, FrenchRP.getPhrase("unable", "/sleep", FrenchRP.getPhrase("frozen")))
        return ""
    end

    if player.Sleeping and IsValid(player.SleepRagdoll) then
        local frozen = player:IsFrozen()
        player.OldHunger = player:getFrenchRPVar("Energy")
        ...
        if player:getFrenchRPVar("Energy") then
            player:setSelfFrenchRPVar("Energy", player.OldHunger)
    ...
    elseif not player:IsFrozen() then
        ...
        for k,v in pairs(ents.FindInSphere(player:GetPos(), 30)) do
            if v:GetClass() == "func_door" then
                FrenchRP.notify(player, 1, 4, FrenchRP.getPhrase("unable", "sleep", "func_door exploit"))
                return ""
            end
        end
...
FrenchRP.defineChatCommand("sleep", FrenchRP.toggleSleep)
FrenchRP.defineChatCommand("wake", FrenchRP.toggleSleep)
FrenchRP.defineChatCommand("wakeup", FrenchRP.toggleSleep)
