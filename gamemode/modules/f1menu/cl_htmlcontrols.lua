...
function PANEL:Init()
    ...
    self:AddFunction("frenchrp", "urlLoaded", function(url)
        self:urlLoaded(url)
    end)
end
...
function PANEL:Think()
    ...
    elseif not self.loaded and not self:IsLoading() then
        self.loaded = true
        self:QueueJavascript([[frenchrp.urlLoaded(document.location.href)]])
    end
end
...
function PANEL:SetHTML(html)
    ...
    self.HTML.urlLoaded = function(panel, url)
        ...
        self.AddressBar:SetText(FrenchRP.deLocalise(url))
