local function registerCommandDefinition(cmd, callback)
    local chatcommands = FrenchRP.getChatCommands()
...
function FrenchRP.defineChatCommand(cmd, callback)
...
function FrenchRP.definePrivilegedChatCommand(cmd, priv, callback, extraInfoTbl)
    cmd = string.lower(cmd)

    local function onCAMIResult(ply, arg, hasAccess, reason)
        if hasAccess then return callback(ply, arg) end

        local notify = ply:EntIndex() == 0 and print or fp{FrenchRP.notify, ply, 1, 4}
        notify(FrenchRP.getPhrase("no_privilege"))
    end
...
local function RP_PlayerChat(ply, text, teamonly)
    FrenchRP.log(ply:Nick() .. " (" .. ply:SteamID() .. "): " .. text)
    local callback = ""
    local DoSayFunc
    local groupSay = FrenchRP.getChatCommand("g")
    local tblCmd = fn.Compose{ -- Extract the chat command
        FrenchRP.getChatCommand,
        string.lower,
    ...
    if string.sub(text, 1, 1) == GAMEMODE.Config.chatCommandPrefix and tblCmd then
        ...
        args = tblCmd.tableArgs and FrenchRP.explodeArg(args) or args
...
local function RP_ActualDoSay(ply, text, callback)
    ...
    if GAMEMODE.Config.alltalk then
        for k, v in pairs(player.GetAll()) do
            FrenchRP.talkToPerson(v, col, callback .. ply:Name(), col2, text, ply)
        end
    else
        FrenchRP.talkToRange(ply, callback .. ply:Name(), text, 250)
    end
    return ""
end

function GM:canChatCommand(ply, cmd, ...)
    if not ply.FrenchRPUnInitialized then return true end

    FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("data_not_loaded_one"))
    FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("data_not_loaded_two"))

    return false
end

GM.OldChatHooks = GM.OldChatHooks or {}
function GM:PlayerSay(ply, text, teamonly) -- We will make the old hooks run AFTER FrenchRP's playersay has been run.
...
local function ReplaceChatHooks()
    ...
    for k,v in pairs(hook.GetTable().PlayerSay) do -- Remove all PlayerSay hooks, they all interfere with FrenchRP's PlayerSay
    ...
    fn.ForEach(warning, FrenchRP.getIncompleteChatCommands())
end
...
local function ConCommand(ply, _, args)
    ...
    local tbl = FrenchRP.getChatCommand(cmd)
...
concommand.Add("frenchrp", ConCommand)
