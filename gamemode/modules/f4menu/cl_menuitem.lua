...
local frenchgray = Color(50, 50, 50, 255)
function PANEL:Paint(w, h)
    ...
    draw.RoundedBox(4, x, y, w, h, disabled and frenchgray or black) -- background

    draw.RoundedBoxEx(4, h, h - 10 + y, w - h + x, 10,
        self.FrenchRPItem and self.FrenchRPItem.buttonColor or not disabled and (self:GetBorderColor() or black) or frenchgray,
        false, false, false, true) -- the colored bar

    draw.RoundedBoxEx(4, x, y, h, h, disabled and frenchgray or gray, true, false, false, false) -- gray box for the model
end
...
-- For overriding
function PANEL:setFrenchRPItem(item)
    self.FrenchRPItem = item
end
...
function PANEL:setFrenchRPItem(job)
    self.BaseClass.setFrenchRPItem(self, job)
...
function PANEL:DoDoubleClick()
    ...
    local job = self.FrenchRPItem
    if job.vote or job.RequiresVote and job.RequiresVote(LocalPlayer(), job.team) then
        RunConsoleCommand("frenchrp", "vote" .. job.command)
    else
        RunConsoleCommand("frenchrp", job.command)
    end

    timer.Simple(1, function() FrenchRP.getF4MenuPanel():Refresh() end)
end

function PANEL:Refresh()
    self:SetTextRight(string.format("%s/%s", team.NumPlayers(self.FrenchRPItem.team), getMaxOfTeam(self.FrenchRPItem)))

    local canGet, important = canGetJob(self.FrenchRPItem)
    self:SetDisabled(not canGet, important)
end
...
function PANEL:setFrenchRPItem(item)
    ...
    self.BaseClass.setFrenchRPItem(self, item)
    self:SetBorderColor(Color(140, 0, 0, 180))
    self:SetModel(item.model)
    self:SetText(item.label or item.name)
    self:SetTextRight(FrenchRP.formatMoney(cost))
end

function PANEL:updatePrice(price)
    if not price then return end

    self:SetTextRight(FrenchRP.formatMoney(price))
end
...
function PANEL:setFrenchRPItem(item)
    ...
    self.DoClick = fn.Partial(RunConsoleCommand, "FrenchRP", "buy", self.FrenchRPItem.name)
end
