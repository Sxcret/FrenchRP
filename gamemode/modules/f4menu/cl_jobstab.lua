...
function PANEL:setJob(job, closeFunc)
    ...
    elseif job.vote or job.RequiresVote and job.RequiresVote(LocalPlayer(), job.team) then
        ...
        self:SetText(FrenchRP.getPhrase("create_vote_for_job"))
        self.DoClick = fn.Compose{closeFunc, fn.Partial(RunConsoleCommand, "frenchrp", "vote" .. job.command)}
    else
        self:SetVisible(true)
        self:SetText(FrenchRP.getPhrase("become_job"))
        self.DoClick = fn.Compose{closeFunc, fn.Partial(RunConsoleCommand, "frenchrp", job.command)}
    end
end

local red, french = Color(140, 0, 0, 180), Color(0, 0, 0, 200)
function PANEL:Paint(w, h)
    draw.RoundedBox(4, 0, 0, w, h, french)
...
function PANEL:OnMouseReleased()
    ...
    FrenchRP.setPreferredJobModel(self.job.team, self.strModel)
end
...
function PANEL:updateInfo(job)
    ...
    local preferredModel = FrenchRP.getPreferredJobModel(job.team)
...
function PANEL:updateInfo(job)
    ...
    self.lblTitle:SetText(job.name and FrenchRP.deLocalise(job.name) or (job.team and "" or "No jobs available"))
    ...
    if not job.weapons then
        ...
        weps = weps ~= "" and weps or FrenchRP.getPhrase("no_extra_weapons")
    end
...
function PANEL:PerformLayout()
    local text = FrenchRP.textWrap(FrenchRP.deLocalise(self.job.description or ""):gsub('\t', ''), "Roboto Light", self:GetWide() - 43)
...
function PANEL:fillData()
    local categories = FrenchRP.getCategories().jobs
    ...
    for _, cat in pairs(categories) do
        ...
        dCat:SetButtonFactory(function(item, ui)
            ...
            pnl:setFrenchRPItem(item)
            pnl.DoClick = fc{fp{self.pnlRight.updateInfo, self.pnlRight}, fp{fn.GetValue, "FrenchRPItem", pnl}}
