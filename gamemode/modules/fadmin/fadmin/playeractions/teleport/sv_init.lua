...
local function Teleport(ply, cmd, args)
    ...
    for _, target in pairs(targets) do
        if IsValid(target) and target:Alive() then
            ...
            local InitialPosition = FrenchRP.findEmptyPos(trace.HitPos + offset, {ply}, 600, 20, Vector(16, 16, 64))
...
local function Bring(ply, cmd, args)
    ...
    for _, target in pairs(targets) do
        ...
        timer.Simple(0, function()
            ...
            target:SetPos(FrenchRP.findEmptyPos(BringTo:GetPos(), {target}, 600, 30, Vector(16, 16, 64)))
...
local function Goto(ply, cmd, args)
    ...
    ply:SetPos(FrenchRP.findEmptyPos(target:GetPos(), {ply}, 600, 30, Vector(16, 16, 64)))
