...
sql.Query([[CREATE TABLE IF NOT EXISTS frenchp_playermodels(
...
Interface functions
---------------------------------------------------------------------------*/
function FrenchRP.setPreferredJobModel(teamNr, model)
    local job = RPExtraTeams[teamNr]
    if not job then return end
    preferredModels[job.command] = model
    sql.Query(string.format([[REPLACE INTO frenchp_playermodels VALUES(%s, %s);]], sql.SQLStr(job.command), sql.SQLStr(model)))

    net.Start("FrenchRP_preferredjobmodel")
        net.WriteUInt(teamNr, 8)
        net.WriteString(model)
    net.SendToServer()
end

function FrenchRP.getPreferredJobModel(teamNr)
    local job = RPExtraTeams[teamNr]
    if not job then return end
...
local function sendModels() -- run after the jobs have loaded
    net.Start("FrenchRP_preferredjobmodels")
...
do
    local models = sql.Query([[SELECT jobcmd, model FROM frenchp_playermodels;]])
