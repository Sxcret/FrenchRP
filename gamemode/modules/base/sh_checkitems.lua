...
local uniqueJob = function(v, tbl)
    local job = FrenchRP.getJobByCommand(v)
...
FrenchRP.validateJob = fn.FAnd{baseSchema, tc.checkTable{
    color =
        tc.addHint(
            tc.tableOf(isnumber),
    ...
    ammo =
        tc.addHint(
            ...
            {"See example on http://wiki.frenchrp.com/index.php/FrenchRP:CustomJobFields"}
        ),
...
FrenchRP.validateShipment = fn.FAnd{buyableSchema, tc.checkTable{
    entity =
        tc.addHint(
...
FrenchRP.validateVehicle = fn.FAnd{buyableSchema, tc.checkTable{
    name =
        tc.addHint(
            isstring,
...
FrenchRP.validateEntity = fn.FAnd{buyableSchema, tc.checkTable{
    ent =
        tc.addHint(
            isstring,
...
local overlappingAgendaCheck = function(t, tbl)
    local agenda = FrenchRP.getAgendas()[t]
...
FrenchRP.validateAgenda = tc.checkTable{
    Title =
        tc.addHint(
            isstring,
            ...
            fn.FAnd{ -- Checks for a table of valid teams that do not already have an agenda assigned
                tc.addHint(
                    ...
                    "The Listeners must be a table of existing teams.",
                    {
                        ...
                        [[Like so: FrenchRP.createAgenda("Some agenda", {TEAM_MANAGER1, TEAM_MANAGER2}, {TEAM_LISTENER1, TEAM_LISTENER2})]]
                    }
                ),
...
FrenchRP.validateCategory = tc.checkTable{
    name =
        tc.addHint(
            isstring,
