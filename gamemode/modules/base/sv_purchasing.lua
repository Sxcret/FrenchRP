function FrenchRP.hooks:canBuyPistol(ply, shipment)
    ...
    if ply:isArrested() then
        return false, false, FrenchRP.getPhrase("unable", "/buy", "")
    end
    ...
    if not ply:canAfford(price) then
        return false, false, FrenchRP.getPhrase("cant_afford", "/buy")
    end
...
local function BuyPistol(ply, args)
    ...
    if not GAMEMODE.Config.enablebuypistol then
        FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("disabled", "/buy", ""))
        return ""
    end
    ...
    local shipment = FrenchRP.getShipmentByName(args)
    if not shipment or not shipment.separate then
        FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("unavailable", "weapon"))
        return ""
    end

    local canbuy, suppress, message, price = hook.Call("canBuyPistol", FrenchRP.hooks, ply, shipment)

    if not canbuy then
        message = message or FrenchRP.getPhrase("incorrect_job", "/buy")
        if not suppress then FrenchRP.notify(ply, 1, 4, message) end
        return ""
    end
    ...
    if IsValid(weapon) then
        ply:addMoney(-cost)
        FrenchRP.notify(ply, 0, 4, FrenchRP.getPhrase("you_bought", args, FrenchRP.formatMoney(cost)))
    else
        FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("unable", "/buy", args))
    end

    return ""
end
FrenchRP.defineChatCommand("buy", BuyPistol, 0.2)

function FrenchRP.hooks:canBuyShipment(ply, shipment)
    if not GAMEMODE:CustomObjFitsMap(shipment) then
        return false, false, "Custom object does not fit map"
    end

    if ply.LastShipmentSpawn and ply.LastShipmentSpawn > (CurTime() - GAMEMODE.Config.ShipmentSpamTime) then
        return false, false, FrenchRP.getPhrase("shipment_antispam_wait")
    end

    if ply:isArrested() then
        return false, false, FrenchRP.getPhrase("unable", "/buyshipment", "")
    end
    ...
    if not canbecome then
        return false, false, FrenchRP.getPhrase("incorrect_job", "/buyshipment")
    end

    local cost = shipment.getPrice and shipment.getPrice(ply, shipment.price) or shipment.price

    if not ply:canAfford(cost) then
        return false, false, FrenchRP.getPhrase("cant_afford", "shipment")
    end
...
local function BuyShipment(ply, args)
    ...
    local found, foundKey = FrenchRP.getShipmentByName(args)
    if not found or found.noship or not GAMEMODE:CustomObjFitsMap(found) then
        FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("unavailable", "shipment"))
        return ""
    end

    local canbuy, suppress, message, price = hook.Call("canBuyShipment", FrenchRP.hooks, ply, found)
    ...
    if IsValid(crate) then
        ...
        FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("unable", "/buyshipment", arg))
    end
...
FrenchRP.defineChatCommand("buyshipment", BuyShipment)

function FrenchRP.hooks:canBuyVehicle(ply, vehicle)
    if not GAMEMODE:CustomObjFitsMap(vehicle) then
        return false, false, "Custom object does not fit map"
    end

    if ply:isArrested() then
        return false, false, FrenchRP.getPhrase("unable", "/buyammo", "")
    end

    if vehicle.allowed and not table.HasValue(vehicle.allowed, ply:Team()) then
        return false, false, FrenchRP.getPhrase("incorrect_job", "/buyvehicle")
    end
    ...
    if GAMEMODE.Config.maxvehicles and ply.Vehicles >= GAMEMODE.Config.maxvehicles then
        return false, false, FrenchRP.getPhrase("limit", "vehicle")
    end

    local cost = vehicle.getPrice and vehicle.getPrice(ply, vehicle.price) or vehicle.price
    if not ply:canAfford(cost) then
        return false, false, FrenchRP.getPhrase("cant_afford", "vehicle")
    end
...
local function BuyVehicle(ply, args)
    ...
    if not found then
        FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("unavailable", "vehicle"))
        return ""
    end

    local Vehicle = FrenchRP.getAvailableVehicles()[found.name]
    if not Vehicle then FrenchRP.notify(ply, 1, 4, "Incorrect vehicle, fix your vehicles.") return "" end

    local canbuy, suppress, message, price = hook.Call("canBuyVehicle", FrenchRP.hooks, ply, found)
    ...
    ply:addMoney(-cost)
    FrenchRP.notify(ply, 0, 4, FrenchRP.getPhrase("you_bought", found.label or found.name, FrenchRP.formatMoney(cost)))
    ...
    if not ent then
        FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("unable", "/buyvehicle", ""))
        return ""
    end
...
FrenchRP.defineChatCommand("buyvehicle", BuyVehicle)

function FrenchRP.hooks:canBuyAmmo(ply, ammo)
    ...
    if ammo.allowed and not table.HasValue(ammo.allowed, ply:Team()) then
        return false, false, FrenchRP.getPhrase("incorrect_job", "/buyammo")
    end
    ...
    if not ply:canAfford(cost) then
        return false, false, FrenchRP.getPhrase("cant_afford", "ammo")
    end
...
local function BuyAmmo(ply, args)
    ...
    if GAMEMODE.Config.noguns then
        FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("disabled", "ammo", ""))
        return ""
    end
    ...
    if not found then
        FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("unavailable", "ammo"))
        return ""
    end

    local canbuy, suppress, message, price = hook.Call("canBuyAmmo", FrenchRP.hooks, ply, found)
    ...
    FrenchRP.notify(ply, 0, 4, FrenchRP.getPhrase("you_bought", found.name, FrenchRP.formatMoney(cost)))
    ply:addMoney(-cost)
...
FrenchRP.defineChatCommand("buyammo", BuyAmmo, 1)
...
local function SetPrice(ply, args)
    ...
    if IsValid(tr.Entity) and tr.Entity.CanSetPrice and tr.Entity.SID == ply.SID then
        ...
        FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("must_be_looking_at", "any lab"))
    end
    return ""
end
FrenchRP.defineChatCommand("price", SetPrice)
FrenchRP.defineChatCommand("setprice", SetPrice)
