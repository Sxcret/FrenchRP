...
function GM:playerBuyDoor(ply, ent)
    if ply:getJobTable().hobo then
        return false, FrenchRP.getPhrase("door_hobo_unable")
    end
...
function GM:FrenchRPVarChanged(ply, var, oldvar, newvalue)
...
function GM:PlayerSpawnedProp(ply, model, ent)
    ...
    if GAMEMODE.Config.proppaying then
        if ply:canAfford(GAMEMODE.Config.propcost) then
            FrenchRP.notify(ply, 0, 4, FrenchRP.getPhrase("deducted_money", FrenchRP.formatMoney(GAMEMODE.Config.propcost)))
            ply:addMoney(-GAMEMODE.Config.propcost)
        else
            FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("need_money", FrenchRP.formatMoney(GAMEMODE.Config.propcost)))
...
local function checkAdminSpawn(ply, configVar, errorStr)
    ...
    if (config == true or config == 1) and ply:EntIndex() ~= 0 and not ply:IsAdmin() then
        FrenchRP.notify(ply, 1, 5, FrenchRP.getPhrase("need_admin", FrenchRP.getPhrase(errorStr) or errorStr))
        return false
    elseif config == 2 and ply:EntIndex() ~= 0 and not ply:IsSuperAdmin() then
        FrenchRP.notify(ply, 1, 5, FrenchRP.getPhrase("need_sadmin", FrenchRP.getPhrase(errorStr) or errorStr))
        return false
    elseif config == 3 and ply:EntIndex() ~= 0 then
        FrenchRP.notify(ply, 1, 5, FrenchRP.getPhrase("disabled", FrenchRP.getPhrase(errorStr) or errorStr, FrenchRP.getPhrase("see_settings")))
        return false
    end
...
function GM:PlayerSpawnedSENT(ply, ent)
    ...
    FrenchRP.log(ply:Nick() .. " (" .. ply:SteamID() .. ") spawned SENT " .. ent:GetClass(), Color(255, 255, 0))
end
...
local function canSpawnWeapon(ply)
    ...
    FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("cant_spawn_weapons"))
...
function GM:PlayerSpawnedVehicle(ply, ent)
    ...
    FrenchRP.log(ply:Nick() .. " (" .. ply:SteamID() .. ") spawned Vehicle " .. ent:GetClass(), Color(255, 255, 0))
end
...
function GM:PlayerSpawnedNPC(ply, ent)
    ...
    FrenchRP.log(ply:Nick() .. " (" .. ply:SteamID() .. ") spawned NPC " .. ent:GetClass(), Color(255, 255, 0))
end
...
function GM:EntityRemoved(ent)
    ...
    if ent.FrenchRPItem and IsValid(owner) then owner:removeCustomEntity(ent.FrenchRPItem) end
...
function GM:OnNPCKilled(victim, ent, weapon)
    ...
    if IsValid(ent) and GAMEMODE.Config.npckillpay > 0 then
        ...
        FrenchRP.notify(ent, 0, 4, FrenchRP.getPhrase("npc_killpay", FrenchRP.formatMoney(GAMEMODE.Config.npckillpay)))
    end
end
...
hook.Add("PlayerInitialSpawn", "FrenchRPCanHearVoice", function(ply)
    timer.Create(ply:UserID() .. "FrenchRPCanHearPlayersVoice", 0.5, 0, fn.Curry(calcPlyCanHearPlayerVoice, 2)(ply))
end)
hook.Add("PlayerDisconnected", "FrenchRPCanHearVoice", function(ply)
    if not ply.DrpCanHear then return end
    for k,v in pairs(player.GetAll()) do
        if not v.DrpCanHear then continue end
        v.DrpCanHear[ply] = nil
    end
    timer.Remove(ply:UserID() .. "FrenchRPCanHearPlayersVoice")
end)
...
function GM:CanPlayerSuicide(ply)
    if ply.IsSleeping then
        FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("unable", "suicide", ""))
        return false
    end
    ...
    if GAMEMODE.Config.wantedsuicide and ply:getFrenchRPVar("wanted") then
        FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("unable", "suicide", ""))
        return false
    end
...
function GM:CanDrive(ply, ent)
    FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("drive_disabled"))
...
function GM:CanProperty(ply, property, ent)
    ...
    FrenchRP.notify(ply, 1, 4, FrenchRP.getPhrase("property_disabled"))
...
function GM:PlayerDeath(ply, weapon, killer)
    ...
    if ply:isArrested() and not GAMEMODE.Config.respawninjail  then
        ...
        FrenchRP.printMessageAll(HUD_PRINTCENTER, FrenchRP.getPhrase("died_in_jail", ply:Nick()))
        FrenchRP.notify(ply, 4, 4, FrenchRP.getPhrase("dead_in_jail"))
    else
    ...
    if GAMEMODE.Config.dropmoneyondeath then
        ...
        if not ply:canAfford(GAMEMODE.Config.deathfee) then
            amount = ply:getFrenchRPVar("money")
        end

        if amount > 0 then
            ply:addMoney(-amount)
            FrenchRP.createMoneyBag(ply:GetPos(), amount)
        end
    end

    if IsValid(ply) and (ply ~= killer or ply.Slayed) and not ply:isArrested() then
        ply:setFrenchRPVar("wanted", nil)
        ply.DeathPos = nil
        ply.Slayed = false
    end
    ...
    FrenchRP.log(ply:Nick() .. " was killed by " .. KillerName .. " with a " .. WeaponName, Color(255, 190, 0))
end
...
function GM:PlayerCanPickupWeapon(ply, weapon)
    ...
    if GAMEMODE.Config.license and not ply:getFrenchRPVar("HasGunlicense") and not ply.RPLicenseSpawn then
...
local function initPlayer(ply)
    timer.Simple(5, function()
        ...
        if GetGlobalBool("FrenchRP_Lockdown") then
            SetGlobalBool("FrenchRP_Lockdown", true) -- so new players who join know there's a lockdown, is this bug still there?
        end
    end)
    ...
    ply:setSelfFrenchRPVar("salary", FrenchRP.retrieveSalary(ply))
...
local function restoreReconnectedEnts(ply)
    ...
    timer.Remove("FrenchRP_removeDisconnected_" .. sid)
    ...
    for _, e in pairs(queuedForRemoval[sid]) do
        ...
        ply:addCustomEntity(e.FrenchRPItem)
    end
...
function GM:PlayerInitialSpawn(ply)
    ...
    FrenchRP.log(ply:Nick() .. " (" .. ply:SteamID() .. ") has joined the game", Color(0, 130, 255))
    ply.FrenchRPVars = ply.FrenchRPVars or {}
...
function GM:PlayerSelectSpawn(ply)
    ...
    local CustomSpawnPos = FrenchRP.retrieveTeamSpawnPos(ply:Team())
    ...
    if ply:isArrested() then
        POS = FrenchRP.retrieveJailPos() or ply.DeathPos -- If we can't find a jail pos then we'll use where they died as a last resort
    end

    -- Make sure the player doesn't get stuck in something
    POS = FrenchRP.findEmptyPos(POS, {ply}, 600, 30, Vector(16, 16, 64))
...
function GM:PlayerSpawn(ply)
    ...
    player_manager.SetPlayerClass(ply, jobTable.playerClass or "player_frenchrp")
    ...
    FrenchRP.log(ply:Nick() .. " (" .. ply:SteamID() .. ") spawned")
end
...
function GM:PlayerLoadout(ply)
    ...
    CAMI.PlayerHasAccess(ply, "FrenchRP_GetAdminWeapons", function(access)
...
local function removeDelayed(entList, ply)
    ...
    timer.Create("FrenchRP_removeDisconnected_" .. sid, removedelay, 1, function()
...
local function collectRemoveEntities(ply)
    ...
    local remClasses = {}
    for _, customEnt in pairs(FrenchRPEntities) do
...
function GM:PlayerDisconnected(ply)
    ...
    FrenchRP.destroyQuestionsWithEnt(ply)
    FrenchRP.destroyVotesWithEnt(ply)

    if isMayor and GetGlobalBool("FrenchRP_LockDown") then -- Stop the lockdown
        FrenchRP.unLockdown(ply)
    end
    ...
    ply:keysUnOwnAll()
    FrenchRP.log(ply:Nick() .. " (" .. ply:SteamID() .. ") disconnected", Color(0, 130, 255))
...
function GM:loadCustomFrenchRPItems()
    ...
    if not GAMEMODE.DefaultTeam or not RPExtraTeams[GAMEMODE.DefaultTeam] then
        ...
        FrenchRP.error("GAMEMODE.DefaultTeam is not set to an existing job.", 1, hints, "lua/frenchrp_customthings/jobs.lua", -1, stack)
    end
end
