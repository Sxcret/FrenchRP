...
local function HMHUD()
    local shouldDraw = hook.Call("HUDShouldDraw", GAMEMODE, "FrenchRP_Hungermod")
    if shouldDraw == false then return end

    local energy = math.ceil(LocalPlayer():getFrenchRPVar("Energy") or 0)
    ...
    if energy > 0 then
        ...
        draw.DrawNonParsedSimpleText(FrenchRP.getPhrase("starving"), "ChatFont", HUDWidth / 2, y - 5, ConVars.StarvingText, 1)
    end
...
hook.Add("HUDDrawTargetID", "HMHUD", HMHUD) --HUDDrawTargetID is called after FrenchRP HUD is drawn in HUDPaint
