...
function meta:drawOwnableInfo()
    ...
    if owned then
        table.insert(doorInfo, FrenchRP.getPhrase("keys_owned_by"))
    end
    ...
    if playerOwned then
        ...
        if allowedCoOwn and not fn.Null(allowedCoOwn) then
            table.insert(doorInfo, FrenchRP.getPhrase("keys_other_allowed"))
    ...
    elseif blocked and superadmin then
        table.insert(doorInfo, FrenchRP.getPhrase("keys_allow_ownership"))
    elseif not blocked then
        table.insert(doorInfo, FrenchRP.getPhrase("keys_unowned"))
        if superadmin then
            table.insert(doorInfo, FrenchRP.getPhrase("keys_disallow_ownership"))
        end
    end
    ...
    if self:IsVehicle() then
        for k,v in pairs(player.GetAll()) do
            ...
            table.insert(doorInfo, FrenchRP.getPhrase("driver", v:Nick()))
            break
        end
    end
...
Door data
---------------------------------------------------------------------------*/
FrenchRP.doorData = FrenchRP.doorData or {}

/*---------------------------------------------------------------------------
Interface functions
---------------------------------------------------------------------------*/
function meta:getDoorData()
    local doorData = FrenchRP.doorData[self:EntIndex()] or {}
...
local function retrieveAllDoorData(len)
    ...
    for i = 1, count do
        ...
        FrenchRP.doorData[ix] = FrenchRP.doorData[ix] or {}

        for vc = 1, varCount do
            local name, value = FrenchRP.readNetDoorVar()
            FrenchRP.doorData[ix][name] = value
        end
    end
end
net.Receive("FrenchRP_AllDoorData", retrieveAllDoorData)
...
local function updateDoorData()
    ...
    FrenchRP.doorData[door] = FrenchRP.doorData[door] or {}

    local var, value = FrenchRP.readNetDoorVar()

    FrenchRP.doorData[door][var] = value
end
net.Receive("FrenchRP_UpdateDoorData", updateDoorData)
...
local function removeDoorVar()
    ...
    local name = id == 0 and net.ReadString() or FrenchRP.getDoorVars()[id].name

    if not FrenchRP.doorData[door] then return end
    FrenchRP.doorData[door][name] = nil
end
net.Receive("FrenchRP_RemoveDoorVar", removeDoorVar)
...
local function removeDoorData()
    local door = net.ReadUInt(32)
    FrenchRP.doorData[door] = nil
end
net.Receive("FrenchRP_RemoveDoorData", removeDoorData)
