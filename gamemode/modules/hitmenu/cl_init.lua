...
hook.Add("HUDPaint", "DrawHitOption", function()
    ...
    if localplayer:isHitman() and localplayer:hasHit() and IsValid(localplayer:getHitTarget()) then
        ...
        local text = FrenchRP.getPhrase("current_hit", localplayer:getHitTarget():Nick())
...
hook.Add("KeyPress", "openHitMenu", function(ply, key)
    ...
    local canRequest, message = hook.Call("canRequestHit", FrenchRP.hooks, hitman, ply, nil, hitman:getHitPrice())

    if not canRequest then
        GAMEMODE:AddNotify(FrenchRP.getPhrase("cannot_request_hit", (message or "")), 1, 4)
        surface.PlaySound("buttons/lightswitch2.wav")
        return
    end

    FrenchRP.openHitMenu(hitman)
end)
...
net.Receive("onHitAccepted", function(len)
    hook.Call("onHitAccepted", FrenchRP.hooks, net.ReadEntity(), net.ReadEntity(), net.ReadEntity())
end)

net.Receive("onHitCompleted", function(len)
    hook.Call("onHitCompleted", FrenchRP.hooks, net.ReadEntity(), net.ReadEntity(), net.ReadEntity())
end)

net.Receive("onHitFailed", function(len)
    hook.Call("onHitFailed", FrenchRP.hooks, net.ReadEntity(), net.ReadEntity(), net.ReadString())
end)
