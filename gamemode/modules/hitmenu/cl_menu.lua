...
function PANEL:Init()
    ...
    self.title:SetText(FrenchRP.getPhrase("hitman"))
    ...
    self.btnRequest:SetText(FrenchRP.getPhrase("hitmenu_request"))
    self.btnRequest.DoClick = function()
        if IsValid(self:GetTarget()) then
            RunConsoleCommand("frenchrp", "requesthit", self:GetTarget():SteamID(), self:GetHitman():UserID())
            self:Remove()
        end
    end

    self.btnCancel = vgui.Create("HitmanMenuButton", self)
    self.btnCancel:SetText(FrenchRP.getPhrase("cancel"))
    self.btnCancel.DoClick = function() self:Remove() end

    self:SetSkin(GAMEMODE.Config.FrenchRPSkin)
...
function PANEL:Think()
    ...
    self.price:SetText(FrenchRP.getPhrase("priceTag", FrenchRP.formatMoney(self:GetHitman():getHitPrice()), ""))
...
function PANEL:PerformLayout()
    ...
    self.name:SetText(FrenchRP.getPhrase("name", self:GetHitman():Nick()))
...
function PANEL:AddPlayerRows()
    ...
    for k, v in pairs(players) do
        local canRequest = hook.Call("canRequestHit", FrenchRP.hooks, self:GetHitman(), LocalPlayer(), v, self:GetHitman():getHitPrice())
...
function PANEL:PerformLayout()
    ...
    self.lblName:SetText(FrenchRP.deLocalise(ply:Nick()))
    self.lblName:SizeToContents()
    self.lblName:SetPos(10, 1)

    self.lblTeam:SetFont("UiBold")
    self.lblTeam:SetText((ply.FrenchRPVars and FrenchRP.deLocalise(ply:getFrenchRPVar("job") or "")) or team.GetName(ply:Team()))
...
Open the hit menu
---------------------------------------------------------------------------*/
function FrenchRP.openHitMenu(hitman)
